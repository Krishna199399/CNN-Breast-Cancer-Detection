...
def patient_history_view():
    ...
    if selected_patient:
        ...
        # Add delete patient button at the top
        st.markdown("---")
        ...
        if st.session_state.get('show_delete_patient_confirm', False):
            st.warning("⚠️ This will permanently delete the patient and all their diagnoses. This action cannot be undone.")
        ...
        if diagnoses.empty:
            ...
            # Add batch selection for diagnoses
            ...
            if st.session_state['selected_diagnoses']:
                if st.button("🗑️ Delete Selected Diagnoses"):
                    st.session_state['show_batch_delete_confirm'] = True
            
            if st.session_state.get('show_batch_delete_confirm', False):
                st.warning(f"⚠️ This will permanently delete {len(st.session_state['selected_diagnoses'])} selected diagnoses. This action cannot be undone.")
                confirm_col1, confirm_col2 = st.columns(2)
                with confirm_col1:
                ...
                with st.expander(f"Diagnosis from {diagnosis['diagnosis_date']}", expanded=True):
                    ...
                    with col1:
                        ...
                        is_selected = st.checkbox(
                            "Select for deletion",
                            key=f"select_{diagnosis['diagnosis_date']}",
                        ...
                        # Add delete diagnosis button
                        if st.button("🗑️ Delete This Diagnosis", key=f"delete_{diagnosis['diagnosis_date']}"):
                            st.session_state[f'show_delete_diagnosis_{diagnosis["diagnosis_date"]}'] = True
                        
                        if st.session_state.get(f'show_delete_diagnosis_{diagnosis["diagnosis_date"]}', False):
                            st.warning("⚠️ This will permanently delete this diagnosis. This action cannot be undone.")
                            confirm_col1, confirm_col2 = st.columns(2)
                            with confirm_col1:
                                if st.button("✅ Yes, Delete", key=f"confirm_delete_{diagnosis['diagnosis_date']}"):
                                    try:
                                        delete_diagnosis(diagnosis['id'])
                                        st.success("Diagnosis deleted successfully!")
                                        st.session_state[f'show_delete_diagnosis_{diagnosis["diagnosis_date"]}'] = False
                                        st.experimental_rerun()
                                    except Exception as e:
                                        st.error(f"Error deleting diagnosis: {e}")
                            with confirm_col2:
                                if st.button("❌ Cancel", key=f"cancel_delete_{diagnosis['diagnosis_date']}"):
                                    st.session_state[f'show_delete_diagnosis_{diagnosis["diagnosis_date"]}'] = False
